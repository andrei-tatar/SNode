...
public:
  ...
  void enableEncryption(const uint8_t *key, const uint8_t *iv, AES_MODE mode);
...
protected:
  ...
  bool write(uint8_t *buffer, uint16_t);
  bool write_to_pipe(uint8_t *buffer, uint16_t node, uint8_t pipe );
  bool enqueue(const uint8_t *frame);
...
private:
  ...
  AES_CTX aes_ctx_encrypt, aes_ctx_decrypt;
};
