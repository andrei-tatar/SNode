...
void RF24Network::update(void)
{
	...
	while (radio.available(&pipe_num) )
	{
		...
		while (!done)
		{
			...
			done = radio.read(buffer, frame_size);

			uint8_t decrypted[frame_size];
			if (encryptionEnabled)
			{
				AES_CTX ctxCopy = aes_ctx_decrypt;
				AES_cbc_decrypt(&ctxCopy, buffer, decrypted, frame_size);
			}
			
			// Read the beginning of the frame as the header
			const RF24NetworkHeader& header = * reinterpret_cast<RF24NetworkHeader*>(encryptionEnabled ? decrypted : buffer);
			...
			if ( header.to_node == node_address )
				...
				enqueue(encryptionEnabled ? decrypted : buffer);
			else
			{
				// Relay it
				write(encryptionEnabled ? buffer : decrypted, header.to_node);
			}
...
bool RF24Network::enqueue(const uint8_t *frame)
{
	bool result = false;
	...
	{
		memcpy(next_frame, frame, frame_size );
...
bool RF24Network::write(RF24NetworkHeader& header, const void* message, size_t len)
{
	...
	if (len)
		memcpy(buffer + sizeof(RF24NetworkHeader),message,min(frame_size-sizeof(RF24NetworkHeader),len));
	...
	else
	{
		if (encryptionEnabled)
		{
			uint8_t encrypted[frame_size];
			AES_CTX ctxCopy = aes_ctx_encrypt; //do a copy			
			AES_cbc_encrypt(&ctxCopy, buffer, encrypted, frame_size);

			return write(encrypted, header.to_node);
		}
		// Otherwise send it out over the air
		return write(buffer, header.to_node);
	}
}

/******************************************************************/

bool RF24Network::write(uint8_t *buffer, uint16_t to_node)
{
	bool ok = false;
	...
	do
	{
		ok = write_to_pipe(buffer, send_node, send_pipe );
	}
	while (!ok && retries--);
...
bool RF24Network::write_to_pipe(uint8_t *buffer, uint16_t node, uint8_t pipe )
{
	bool ok = false;
	...
	do
	{
		ok = radio.write(buffer, frame_size);
	}
	while ( !ok && --attempts );
...
#ifdef SERIAL_DEBUG
	//sprintf_s(buffer,sizeof(buffer),"id %04x from 0%o to 0%o type %c",id,from_node,to_node,type);
#endif
	return buffer;
}
...
void RF24Network::enableEncryption(const uint8_t *key, const uint8_t *iv, AES_MODE mode)
{
	encryptionEnabled = true;

	AES_set_key(&aes_ctx_encrypt, key, iv, mode);
	AES_set_key(&aes_ctx_decrypt, key, iv, mode);
	AES_convert_key(&aes_ctx_decrypt);
}

void RF24Network::disableEncryption()
{
