...
uint8_t RF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
  ...
  CSN_LOW();
  status = spi_ReadWriteByte( R_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
...
{
  CSN_LOW();
  spi_ReadWriteByte( R_REGISTER | ( REGISTER_MASK & reg ) );
  uint8_t result = spi_ReadWriteByte(0xff);
...
uint8_t RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  CSN_LOW();
  status = spi_ReadWriteByte( W_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
...
uint8_t RF24::write_payload(const void* buf, uint8_t len)
{
  ...
  //debugmsg("[Writing %u bytes %u blanks]",data_len,blank_len);
  
  CSN_LOW();
  status = spi_ReadWriteByte( W_TX_PAYLOAD );
  while ( data_len-- )
    spi_ReadWriteByte(*current++);
  while ( blank_len-- )
    spi_ReadWriteByte(0);
  CSN_HIGH();
...
uint8_t RF24::read_payload(void* buf, uint8_t len)
{
  ...
  //debugmsg("[Reading %u bytes %u blanks]",data_len,blank_len);
  
  CSN_LOW();
  status = spi_ReadWriteByte( R_RX_PAYLOAD );
  while ( data_len-- )
    *current++ = spi_ReadWriteByte(0xff);
  while ( blank_len-- )
...
uint8_t RF24::flush_rx(void)
{
  ...
  CSN_LOW();
  status = spi_ReadWriteByte( FLUSH_RX );
  CSN_HIGH();
...
uint8_t RF24::flush_tx(void)
{
  ...
  CSN_LOW();
  status = spi_ReadWriteByte( FLUSH_TX );
  CSN_HIGH();
...
void RF24::begin(void)
{
  ...
  write_register(INT_STATUS,_BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
bool RF24::write( const void* buf, uint8_t len )
{
  ...
  uint8_t observe_tx;
  uint8_t status;
  uint32_t sent_at = getTime();
  const uint32_t timeout = 500; //ms to wait for timeout
  do
  {
    status = read_register(OBSERVE_TX,&observe_tx,1);
  }
  while( ! ( status & ( _BV(TX_DS) | _BV(MAX_RT) ) ) && ( getTime() - sent_at < timeout ) );
...
bool RF24::available(uint8_t* pipe_num)
{
  ...
  if (result)
  {
    ...
    if ( pipe_num )
      *pipe_num = ( status >> RX_P_NO ) & 0xB111;
    ...
    write_register(INT_STATUS,_BV(RX_DR) );

    // Handle ack payload receipt
    if ( status & _BV(TX_DS) )
    {
      write_register(INT_STATUS,_BV(TX_DS));
    }
  }
...
void RF24::whatHappened(bool& tx_ok,bool& tx_fail,bool& rx_ready)
{
  ...
  uint8_t status = write_register(INT_STATUS,_BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  CSN_LOW();
  spi_ReadWriteByte( W_ACK_PAYLOAD | ( pipe & 0B111 ) );
  const uint8_t max_payload_size = 32;
