...
{
	uint32_t tin[4], xorr[4], buf[4];
	AES_hncpy32(xorr,(uint32_t *)(ctx->iv));
	for (length -= AES_BLOCKSIZE; length >= 0; length -= AES_BLOCKSIZE)
	{
		AES_hncpy32(tin,(uint32_t *)msg);
		memcpy(buf,tin,AES_BLOCKSIZE);
		AES_decrypt(ctx, buf);
		((uint32_t *)out)[0] = htonl(buf[0] ^ xorr[0]);
		((uint32_t *)out)[1] = htonl(buf[1] ^ xorr[1]);
		((uint32_t *)out)[2] = htonl(buf[2] ^ xorr[2]);
		((uint32_t *)out)[3] = htonl(buf[3] ^ xorr[3]);
		memcpy(xorr,tin,AES_BLOCKSIZE);
